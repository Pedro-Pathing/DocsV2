...
- **setHeadingInterpolation(HeadingInterpolator):** Control the robot's heading while following paths. (See [Interpolation](/en/reference/interpolation))
- **setBrakingStrength(double):** Control deceleration (See [Deceleration](/en/reference/deceleration))
- **setTValueConstraint(double):** Control when each path is considered complete (See [Path Constraints](/en/reference/constraints) for more on timing.)
- **build():** Convert PathBuilder -> PathChain

Curious how Pedro actually follows these paths? Dive into the [Drive Vector Algorithm](/en/reference/drive-vector-algorithm) for the details.
